...
def compute_images(image, confidence, detections):
    ...
    cv2.rectangle(image, (sX, sY), (eX, eY),
                  (0, 0, 255), 2)
    element = {"iname": imagePath[len(path)+1:], "bbox": [int(sX), int(sY), int(eX - sX), int(eY - sY)]}
...
path = input("Please enter the image folder path ")
imagePaths = list(list_files(path, validExts=image_types, contains=None))
d_n_net = cv2.dnn.readNetFromCaffe("./Model_Files/deploy.prototxt.txt", "./Model_Files/res10_300x300_ssd_iter_140000.caffemodel")
json_list = []
